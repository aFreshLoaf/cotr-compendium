...
function BlockBody({ value, editMode, onChange, placeholder, content, goTo }) {
  ...
  const addLink = (bi) => {
    const items = [...(blocks[bi].items || []), { kind: 'internal', section: 'subclasses', id: '' }];
    updateBlock(bi, { items });
  };
  ...
  // ── Resolve a link's display label and click target ─────────────
  // Internal links resolve to an entry in `content`; external links carry their
  // own `url` and `label`. Items default to `kind: 'internal'` for back-compat.
  const resolveLink = (item) => {
    const kind = item.kind || 'internal';
    if (kind === 'external') {
    ...
    return { kind: 'internal', label: entry.name, section: item.section, id: item.id };
  };
      ...
      {blocks.map((b, i) => (
        <div key={i} style={{ marginBottom: '10px', position: 'relative' }}>
          ...
          {b.type === 'links' && (() => {
            ...
            const setKind = (li, val) => {
              if (val === 'external') {
                updateLink(i, li, { kind: 'external', url: '', label: '', section: undefined, id: undefined });
              } else {
                updateLink(i, li, { kind: 'internal', section: val, id: '', url: undefined, label: undefined });
              }
            };
            return (
                    ...
                    {items.map((item, li) => {
                      ...
                      return (
                        <div key={li} style={{ display: 'flex', gap: '6px', alignItems: 'center', marginBottom: '6px' }}>
                          <select value={kindValue(item)}
                            onChange={(e) => setKind(li, e.target.value)}
                            style={{ ...styles.textarea, minHeight: 'unset', padding: '4px 8px', width: '130px' }}>
                          ...
                          {isExternal ? (
                            <>
                              <input value={item.label || ''} placeholder="Label (e.g. D&D Beyond)"
                                onChange={(e) => updateLink(i, li, { label: e.target.value })}
                                style={{ ...styles.textarea, minHeight: 'unset', padding: '4px 8px', flex: '1 1 30%' }} />
                              <input value={item.url || ''} placeholder="https://…"
                                onChange={(e) => updateLink(i, li, { url: e.target.value })}
                                style={{ ...styles.textarea, minHeight: 'unset', padding: '4px 8px', flex: '1 1 50%' }} />
                            </>
                          ) : (
                    ...
                    {items.map((item, li) => {
                      ...
                      const handleClick = () => {
                        if (resolved.kind === 'external') {
                          ...
                          goTo(resolved.section, resolved.id);
                        }
                      };
                      const indicator = resolved.kind === 'external' ? '↗' : '→';
                      return (
                        <button key={li}
                          onClick={handleClick}
                          title={resolved.kind === 'external' ? resolved.url : undefined}
                          style={{
                            background: '#5c1414',
                            color: '#f5ecd9',
                            border: '1px solid #8b6914',
